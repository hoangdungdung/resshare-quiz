...
import com.resshare.quiz.RefFireBaseQuiz;

public class QuizBaseListener extends LoadFormQuizBaseListener {
	...
	public String getReferenceNamePostData() {
		...
		return ResFirebaseReference.getInputPathReference(RefFireBaseQuiz.requets_informatics);
		// "../requets/searching");
	}
		...
		try {
			if (snapshot1.child("processing").getValue() == null) {
				...
				FirebaseDatabase.getInstance()
						.getReference(ResFirebaseReference.getAbsolutePath("../data/questions/" + subject))
						.addListenerForSingleValueEvent(new ValueEventListener() {
							...
							public void onDataChange(DataSnapshot snapshotquestions) {
								...
								if (snapshotquestions.exists()) {
									...
									DataSnapshot objSnapshotquestions_name_detail = getQuestionsDetailt(
											questions_name_master, snapshot_question_list);
									// answer_choose
									String examination_key = FirebaseDatabase.getInstance()
											.getReference(ResFirebaseReference.getAbsolutePath(
													"../data/test/examination/" + subject + "/" + user_id))
											.push().getKey();

									sendMessageQuestionView(snapshot1, user_id, questions_name_master,
											arrayQuestionsNameNext, objSnapshotquestions_name_detail, examination_key,
											1, subject, "", "");
									// question_1
	...
	public void sendMessageQuestionView(DataSnapshot snapshot11, String user_id, String questions_name_master,
			...
			int order_number, String subject, String questions_type, String questions_key) {
	...
	public void sendMessageQuestionView3(DataSnapshot snapshot11, String user_id, String questions_name_master,
			String arrayQuestionsNameNext, DataSnapshot objSnapshotquestions_name_detail, String examination_key,
	...
	public static void viewListQuestion(DataSnapshot snapshotRoot, String subject, String pathCollection) {
		try {
			...
			FirebaseDatabase.getInstance().getReference(ResFirebaseReference.getAbsolutePath(pathCollection))
					.addListenerForSingleValueEvent(new ValueEventListener() {
							...
							if (snapshotGrade.exists()) {
								...
								for (DataSnapshot snapshotType : snapshotGrade.getChildren()) {
									...
									for (DataSnapshot snapshotKey : snapshotType.getChildren()) {
										...
										String layout_form = "../configuration/system_setting/layout/android/form/test_"
												+ subject;
										gridItem.put("layout_form", layout_form);
										gridItem.put("fragment",
												"com.resshare.core.screen.DynamicFragmentRecyclerViewV2");
								...
								Object cfgLayout = com.resshare.framework.core.service.Cache.configuration.child(path)
										.getValue();
								String pathItem = "system_setting/layout/android/quiz_grid_list_question/grid_view_layout_item";

								Object cfgLayoutItem = com.resshare.framework.core.service.Cache.configuration
										.child(pathItem).getValue();
