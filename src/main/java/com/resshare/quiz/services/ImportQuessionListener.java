...
public class ImportQuessionListener extends QuizBaseListener {
	...
	private void insertIcon_main_dashboard(JSONObject jsonObjectRoot) {
		...
		String pathquestion_icon_main_dashboard = ResFirebaseReference
				.getAbsolutePath("../configuration/system_setting/layout/android/main_dashboard/grid_view_data/");// list_item");
		FirebaseDatabase.getInstance().getReference(pathquestion_icon_main_dashboard)
					...
					public void onDataChange(DataSnapshot snapshot) {
						if (snapshot.exists()) {
							...
							FirebaseDatabase.getInstance()
									.getReference(pathquestion_icon_main_dashboard + "/list_item/" + String.valueOf(l))
									.setValue(icon_main_dashboard);
	...
	private void insertQuestion(DataSnapshot snapshotRoot, String user_id, JSONObject jsonObjectRoot) {
		...
		if ("".equals(questions_id)) {
			...
			FirebaseDatabase.getInstance().getReference(pathsubject + "/" + questions_id)
					.addListenerForSingleValueEvent(new ValueEventListener() {
						...
						public void onDataChange(DataSnapshot snapshot) {
							if (snapshot.exists()) {
								...
								if (user_id.equals(user_id_owner)) {
									...
									FirebaseDatabase.getInstance().getReference(pathquestion_type + "/" + questions_id)
											.setValue(hm);
